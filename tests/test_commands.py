...
scenarii = [
    ...
    #
    (
        """json validate --schema-file=fixtures/three.schema.json < fixtures/three.data1.json""",
        False,
    ),
    (
        """json validate --schema-file=fixtures/three.schema.json < fixtures/three.data2.json""",
        True,
    ),
]
...
def test_cli_add(pointer, document, fragment, success, result):
    ...
    cmd = cli.AddCommand()
    runner(
        cmd, [pointer, "--document-json", doc, "--fragment-json", frag], success, result
    )
...
def test_cli_move(pointer, document, target, success, result):
    ...
    runner(
        cmd,
        [pointer, "--document-json", doc, "--target-pointer", target],
        success,
        result,
    )
