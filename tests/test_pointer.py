...
class TestPointer(TestCase):
    document = {"foo": ["bar", "baz", {"$ref": "obj2#/sub"}]}
    ...
    def test_bypass_reference(self):
        assert "obj2#/sub" == extract(self.document, "/foo/2/$ref", bypass_ref=True)
...
class TestSequence(TestCase):
    ...
    collections_document = MySequenceType(
        ["foo", "bar", MyMappingType({"$ref": "baz"})]
    )
    ...
    def test_bypass_ref(self):
        assert self.document[2] == extract(self.document, "/2", bypass_ref=True)
...
class TestMapping(TestCase):
    ...
    def test_bypass_ref(self):
        assert self.document["bar"] == extract(self.document, "/bar", bypass_ref=True)
...
class MyMappingType(TestCase):
    document = MyMappingType(
        {"foo": 42, "bar": MyMappingType({"$ref": "baz"}), 4: True}
    )
